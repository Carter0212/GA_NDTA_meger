...
#define crossover_flag 1 // if true, crossover will choose multi cbsd, else, only choose single cbsd
...
struct bandwidth_parameters bandwidth_ind;
//------------output result------------
...
void set_bandwidth_serial_number(int band_num){
	int H_m_c = 0; // for count by channel have how many cbsd
	
	int C_n; // for count
	//set cbsd channel all is 0
	for(C_n=0;C_n<cbsd_num;C_n++){
		for(int SetChannel = 0;SetChannel<cbsd_bandwidth;SetChannel++)  cbsd_ind[C_n].enable_channel[SetChannel] = 0;
		
		//bandwith set channel number
		for(band_num= 0; band_num<cbsd_bandwidth ;band_num++){

			if(bandwidth_ind.Channel_Usage_Situation[14] == H_m_c+1){
				H_m_c++;
			}
			

			if(bandwidth_ind.Channel_Usage_Situation[band_num] == H_m_c){
				cbsd_ind[C_n].enable_channel[band_num]=1;
				bandwidth_ind.Channel_Usage_Situation[band_num]++;
				break;  //leave loop beacuse find cbsd's channel
		...
		printf("th%d cbsd channel is: ",C_n);
		for(int teest=0;teest < cbsd_bandwidth;teest++) printf("%d ",cbsd_ind[C_n].enable_channel[teest]);
		printf("\n");
	}
	printf("total channel is: ");
	for(int ttest = 0; ttest < cbsd_bandwidth ; ttest++)printf("%d ",bandwidth_ind.Channel_Usage_Situation[ttest]);
	printf("\n");
}
...
float do_cbsd_sinr(int bs_num, int area, int i, float c_p, float* c_p_r){ // SINR = Prx/(Pi+Ps)
	...
	for(P_c=0;P_c<cbsd_num;P_c++){		
		for(band_num = 0;band_num < cbsd_bandwidth; band_num++){
			if((P_c!=bs_num)&&(cbsd_ind[P_c].enable_channel[band_num]==cbsd_ind[bs_num].enable_channel[band_num])&&(cbsd_ind[P_c].enable_channel[band_num]==1)) P_i+=do_cbsd_P_rx(P_c,area,i,c_p_r[P_c]);
...
float do_throughput(int* area_num, int area_num_max, float* m_a_p, float* c_p){
    ...
    float cbsd_t_buf[cbsd_num]={0.0},cbsd_over[cbsd_num]={0.0}; // buffer for cbsd throughput, show the overload cbsd
...
float do_first_throughput(int* area_num, int area_num_max, float* m_a_p, float* c_p, float max_avg){
    ...
    int area_num_buf[4]={0}; // buffer for area number
    int i,i_c;
	float cbsd_p_sum=0.0; // for calculate near cbsd people number
    float cbsd_t_buf[cbsd_num]={0.0},cbsd_over[cbsd_num]={0.0}; // buffer for cbsd throughput, show the overload cbsd
...
//------------GA initialization------------
void do_GA_initialization(float* mbs_TX_power,int seed_num,int* seed_r){
	int i,a_c,a_p_c,c_c,c_p_c; // for count
	int mbs_r,cbsd_r; //for record
	float mbs_max_r=0.0;
	int cbsd_c=0; // for count
	...
	for(i=0;i<population_size;i++){
		...
		for(a_c=0;a_c<4;a_c++){
			...
			mbs_r=rand() % 5;
			chromosome_ind[i].area[a_c][mbs_r]=1;
			mbs_max_r+=mbs_TX_power[mbs_r];
		}

		for(c_c=0;c_c<cbsd_num;c_c++){
			chromosome_ind[i].mbs_cbsd[c_c]=0;
			for(c_p_c=0;c_p_c<5;c_p_c++){
				chromosome_ind[i].cbsd[c_c][c_p_c]=0;
			}
		}

		while(cbsd_c<(int)((mbs_TX_max-mbs_max_r)/mbs_cbsd_relay_p)&&(cbsd_c<cbsd_num)){
			cbsd_r=rand() % cbsd_num;
			if(chromosome_ind[i].mbs_cbsd[cbsd_r]==0) {
				chromosome_ind[i].mbs_cbsd[cbsd_r]=1;
				chromosome_ind[i].cbsd[cbsd_r][rand() % 5]=1;
				cbsd_c++;
			}
		}
		cbsd_c=0;
...
//------------GA fitness function------------
int do_GA_fitness_function(float* mbs_TX_power,float* cbsd_TX_power,int area_num_max,int* area_num,int g_num,float* rec_bs_f_mbs_p,float* rec_bs_f_cbsd_p,FILE *fpw,int seed_num,int* seed_r){
	...
	float good_avg_power[population_size]={0.0},power_buf,max_power=0.0; // record good chromosome to choose top 5 chromosome
	float T=0.0; // SA temperature
	...
	for(i=0;i<population_size;i++){
		...
		if(sum_p<=mbs_TX_max){ // check whether a1~a4 sum power is bigger than max power
			...
			if(max_power<good_avg_power[buf_c]) max_power=good_avg_power[buf_c];
			buf_c++;
			//max_avg=do_throughput(&area_num[0],area_num_max,&m_a_p[0],&c_p[0],max_avg);
		}else{ // SA part
			T=500*pow(cooling_fac,(g_num+1));
			if((rand()%100)<=(exp((fabs(max_power-(do_throughput(&area_num[0],area_num_max,&m_a_p[0],&c_p[0]))))*(-1)/T)*100)){
				good_avg_power[buf_c]=do_throughput(&area_num[0],area_num_max,&m_a_p[0],&c_p[0]); // save the good one's avg. power to arrange
				buf_c++;
			}
		}
	...
	for(i=0;i<population_size;i++){
		...
		for(c_c=0;c_c<cbsd_num;c_c++){
			...
			if(i>=buf_c) chromosome_ind[i].mbs_cbsd[c_c]=0;
			for(c_p_c=0;c_p_c<5;c_p_c++){
...
void data_rate_calculate(int* area_num,int* rec_good_num,float* rec_bs_f_mbs_p,float* rec_bs_f_cbsd_p,FILE *fpw){
	...
	// for record the max cbsd allocation in test times
	float max_cbsd[cbsd_num]={0.0},max_mbs[4]={0.0};
	...
	int data[((2*iteration_num+1)*test_num)*2]; // csv data rows
	int seed_r[(2*iteration_num+1)*test_num]; // seed record data
	FILE *seed=NULL;
	seed=fopen("HA_seed.csv","r");
...
//	while(test_c<test_num){
		...
		do_GA_initialization(&mbs_TX_power[0],seed_num,&seed_r[0]);
		seed_num++; // for next random seed
		while(g_num<iteration_num){
            good_chromosome_num=do_GA_fitness_function(&mbs_TX_power[0],&cbsd_TX_power[0],area_num_max,&area_num[0],g_num,&rec_bs_f_mbs_p[0],&rec_bs_f_cbsd_p[0],fpw,seed_num,&seed_r[0]);
			seed_num++; // for next random seed
		...
		// record the max cbsd allocation in test times
		if(max_avg_t_of_e_g_s!=1) {
			if(max_throughput<max_avg) {
				max_throughput=max_avg;
				for(i=0;i<4;i++) {
					max_mbs[i]=bs_rec.a_p[i];
				}
				for(i=0;i<cbsd_num;i++) {
					max_cbsd[i]=bs_rec.c_p[i];
				}
			}
		}
...
//	}
	...
	// check the max allocation in test times
	if(max_avg_t_of_e_g_s!=1) {
		printf("the max : %f\n",max_throughput);
		printf("check the max allcation\n");
		printf("mbs ");
		for(i=0;i<4;i++) {
			printf("%d = %.3f ",i+1,max_mbs[i]);
		}
		printf("\ncbsd ");
		for(i=0;i<cbsd_num;i++) {
			printf("%d = %.3f, ",i+1,max_cbsd[i]);
		}
	}
...
int main(){
	...
	set_bandwidth_serial_number(band_num); //set bandwidth serial number
