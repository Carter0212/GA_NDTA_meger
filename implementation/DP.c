...
float do_cbsd_sinr(int bs_num, int area, int i, float c_p, float* c_p_r){ // SINR = Prx/(Pi+Ps)
	...
	int band_num; //now bandwdith number us to count
	int P_c; // for count
...
void do_GA_SCM(int good_chromosome_num,int seed_num, int* seed_r){
	...
	if(good_chromosome_num>(elitism_num+1)){
		for(i=elitism_num;i<good_chromosome_num-1;i++){
			if((rand()%100)<mbs_crossover_prob){ // check mbs whether exchange
			...
			if((rand()%100)<cbsd_crossover_prob){ // check cbsd whether exchange
		...
		for(i=elitism_num;i<good_chromosome_num;i++){
			if((rand()%100)<mutation_prob){ // check mbs whether exchange
				rand_buf=rand() % 4; // random select mbs area
				for(a_p_c=0;a_p_c<5;a_p_c++){
...
void do_channel_initialization(){
	int i;
	for(i=0;i<cbsd_bandwidth;i++){
		bandwidth_ind.Channel_Usage_Situation[i] = 0 ;
	}

}

void do_channel_allocation(){
