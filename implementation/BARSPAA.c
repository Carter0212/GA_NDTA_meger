...
//------------output result------------
void write_excel(FILE *fpw){ // write the max average power times result to excel 
	int i;
...
float do_mbs_throughput(int area, int i, float m_p, int* area_change){ // C = B * (log_2(1+SNR))
	float C,SNR;
	SNR=do_mbs_snr(area,i,m_p);
	C=mbs_bandwidth*(log(1+SNR)/log(2)); // unit : Mbps
	if((log(1+SNR)/log(2))<wf_threshold_l) {
		area_change[area]++;
		if(m_p>=mbs_ue_max) C=0;
	}
	if((log(1+SNR)/log(2))>wf_threshold_u) area_change[area]--;
	//printf("throughput: %f\n\n", C);
	return C;
}

// ------------calculate throughput------------
float do_throughput(int* area_num, int area_num_max, float* m_a_p, float* c_p, float max_avg, int* area_change){
	...
	for(i=0;i<area_num_max;i++){ 
		if(area_num_buf[0]!=0){
			ue_buf[0][i].throughput_mbs=do_mbs_throughput(0,i,m_a_p[0],&area_change[0]); // connect mbs
		...
		if(area_num_buf[1]!=0){
			ue_buf[1][i].throughput_mbs=do_mbs_throughput(1,i,m_a_p[1],&area_change[0]);
			for(cbsd_c=0;cbsd_c<cbsd_num;cbsd_c++){
		...
		if(area_num_buf[2]!=0){
			ue_buf[2][i].throughput_mbs=do_mbs_throughput(2,i,m_a_p[2],&area_change[0]);
			for(cbsd_c=0;cbsd_c<cbsd_num;cbsd_c++){
		...
		if(area_num_buf[3]!=0){
			ue_buf[3][i].throughput_mbs=do_mbs_throughput(3,i,m_a_p[3],&area_change[0]);
			for(cbsd_c=0;cbsd_c<cbsd_num;cbsd_c++){
...
void data_rate_calculate(int* area_num, FILE *fpw){
	float mbs_TX_power[5]={5,6.25,7.5,8.75,10}; // unit : w
    float cbsd_TX_power=1.0; // unit : w
	float m_a_p[4],m_c_p[cbsd_num]={0.0}; // mbs area 1~4 and mbs to cbsd power
	float c_p[cbsd_num]={0.0}; // cbsd power
	int i_m,i_c,m_c[4],c_c[cbsd_num],cbsd_c,i; // for count
	float mbs_max_r=0.0;  // record mbs sum power
	int cbsd_r; // record cbsd
	int cal_c=0;
	int area_num_max; // max ue number in 4 area
	float max_avg=0.0; // max average throughput
	float sum_p=0.0; // bs sum power
	int area_change[4]={0}; // water filling judge
	int change_f=0; // change flag
	int test_c=0,test_num=1; // for test
	float test_max_t=0.0; // record the max throughput in test times
	...
	int data[(iteration_num*test_num)*2]; // csv data rows
	int seed_r[iteration_num*test_num]; // seed record data
	FILE *seed=NULL;
	seed=fopen("WF_seed.csv","r");
	...
	while(test_c<test_num) {
		...
		for(i_m=0;i_m<4;i_m++){
			m_a_p[i_m]=mbs_ue_max/2;
		}

		while(cal_c<iteration_num){
			...
			// initial mbs to cbsd & cbsd power
			for(i_m=0;i_m<4;i_m++){
				mbs_max_r+=m_a_p[i_m];
			}
			for(i_c=0;i_c<cbsd_num;i_c++) {
				m_c_p[i_c]=0;
				c_p[i_c]=0;
			}
			...
			while(cbsd_c<(int)((mbs_TX_max-mbs_max_r)/mbs_cbsd_relay_p)&&(cbsd_c<cbsd_num)){
				...
				if(m_c_p[cbsd_r]==0) {
					m_c_p[cbsd_r]=mbs_cbsd_relay_p;
					c_p[cbsd_r]=cbsd_TX_power;
					cbsd_c++;
				}
			}
			printf("mbs to cbsd link check: ");
			for(i_m=0;i_m<4;i_m++) printf("mbs %d : %.3f, ",i_m+1,m_a_p[i_m]);
			for(i_c=0;i_c<cbsd_num;i_c++) printf("cbsd %d : %.3f, ",i_c+1,c_p[i_c]);
			printf("\n");
			...
			if(sum_p<=mbs_TX_max){ // check whether a1~a4 sum power is bigger than max power
				max_avg=do_throughput(&area_num[0],area_num_max,&m_a_p[0],&c_p[0],max_avg,&area_change[0]);
				if(test_num==1) fprintf(fpw,"%f,",max_avg);
			}
			
			for(i_m=0;i_m<4;i_m++) printf("area %d = (%d , %d), ",i_m+1,area_change[i_m],area_num[i_m]);
			printf("\n\n\n");

			// water filling
			for(i_m=0;i_m<4;i_m++) {
				//if((area_change[i_m]>(area_num[i_m]/2))&&(m_a_p[i_m]+1.25<=mbs_ue_max)&&(change_f==0)) {
				if((area_change[i_m]>(area_num[i_m]/2))&&(m_a_p[i_m]+1.25<=mbs_ue_max)) {
					m_a_p[i_m]+=1.25;
					//change_f=1;
				//} else if((area_change[i_m]<-(area_num[i_m]/2))&&(m_a_p[i_m]-1.25>=0)&&(change_f==0)) {
				} else if((area_change[i_m]<-(area_num[i_m]/2))&&(m_a_p[i_m]-1.25>=0)) {
					m_a_p[i_m]-=1.25;
					//change_f=1;
				}
			}
		...
		for(i=0;i<ue_num;i++){
			if(ue_rec[i].connect_bs_num!=0) relay_n+=1;
		}
	...
	printf("\n\navg. max avg. throughput in test times : %f\n\n",test_max_t/test_num);
	fprintf(fpw,"\n\navg. max avg. throughput in test times\n");
	fprintf(fpw,"%f\n",test_max_t/test_num);
...
int main(){
	int area_num[4]={0};
	FILE *fpw=NULL;
	fpw=fopen("BARSPAA.csv","w");
	
	deploy_node(&area_num[0]); // deploy every node
	
	data_rate_calculate(&area_num[0],fpw); // calculate ue's data rate
	
	write_excel(fpw); // write the result into excel
